...
//-----------------------------------------------------------------------------
static U16 DescGeneric_SNDK_MLC_Read(tFIO_JOB* pJob);
static U16 DescGeneric_SNDK_MLC_FastRead(tFIO_JOB *pJob);
static U16 DescGeneric_SNDK_MLC_SlcRead(tFIO_JOB *pJob);
static U16 DescGeneric_SNDK_MLC_IdsRead(tFIO_JOB *pJob);
static U16 DescGeneric_SNDK_MLC_IdsPageRead(tFIO_JOB *pJob);
static U16 DescGeneric_SNDK_MLC_PreOpen(tFIO_JOB* pJob);
static U16 DescGeneric_SNDK_MLC_RR(tFIO_JOB *pJob);
static U16 DescGeneric_SNDK_MLC_Write(tFIO_JOB* pJob);
static U16 DescGeneric_SNDK_MLC_Erase(tFIO_JOB* pJob);
static U16 DescGeneric_SNDK_MLC_Exception(tFIO_JOB* pJob);

static __inline void DescGeneric_SNDK_MLC_OpenPage(tFIO_JOB* pJob, U16 actDie, tPAA paa, U16 descOffset);
static __inline void DescGeneric_SNDK_MLC_InitTemplate(void);

static __inline void DescGeneric_SNDK_MLC_HD_Retry_CW(tFIO_JOB* pJob, U8 fg_cw, U8 fg_fd, U8 fg_JU, tPAA paa);
static __inline void DescGeneric_SNDK_MLC_SD_Retry_CW(tFIO_JOB* pJob, U8 fg_cw, tPAA paa);
...
{
    DescGeneric_SNDK_MLC_Read,
    DescGeneric_SNDK_MLC_FastRead,
    DescGeneric_SNDK_MLC_SlcRead,
	DescGeneric_SNDK_MLC_Exception,
    DescGeneric_SNDK_MLC_IdsPageRead,
    DescGeneric_SNDK_MLC_IdsRead,
    DescGeneric_SNDK_MLC_PreOpen,
	DescGeneric_SNDK_MLC_RR,
    DescGeneric_SNDK_MLC_Exception,
    DescGeneric_SNDK_MLC_Write,
    DescGeneric_SNDK_MLC_Erase,
};
...
U8  gVoltageStageList[MAX_READRETRY_ROW][MAX_READRETRY_COL] = {{0x00, 0x7B, 0x7C, 0x7E}, {0x04, 0x00, 0x04, 0x06},
                                                               {0x08, 0x06, 0x7C, 0x06}, {0x7C, 0x7A, 0x08, 0x00},
                                                               {0x0C, 0x7A, 0x0C, 0x06}, {0x10, 0x06, 0x78, 0x00},
                                                               {0x20, 0x00, 0x74, 0x7A}, {0x1C, 0x7A, 0x70, 0x7A},
                                                               {0x14, 0x74, 0x00, 0x00}, {0x24, 0x74, 0x00, 0x06},
                                                               {0x18, 0x74, 0x00, 0x7A}, {0x04, 0x6E, 0x78, 0x7A},
                                                               {0x0C, 0x06, 0x00, 0x7A}, {0x10, 0x00, 0x04, 0x0C},
                                                               {0x14, 0x7A, 0x08, 0x0C}, {0x18, 0x6E, 0x0C, 0x74}};
...
{
    DescGeneric_SNDK_MLC_InitTemplate();
...
static U16 DescGeneric_SNDK_MLC_Read(tFIO_JOB* pJob)
{
    ...
    for (cnt = 0; cnt < pJob->paaCnt; cnt++)
    {
        ...
        {
            descOffset = DescGeneric_SNDK_MLC_OpenPage(pJob, actDie, paa, descOffset);
        }
...
static __inline void DescGeneric_SNDK_MLC_OpenPage(tFIO_JOB* pJob, U16 actDie, tPAA paa, U16 descOffset)
{
    ...
    if (paa.b.plane)
    {
        ...
        gTplGenericOpenRow.field.OPTION = SEQ_OPTION_0280C8; //wcnt 61 + pcnt51
    ...
    else
    {
        ...
        for (planeOffset = 0; planeOffset < pJob->plane; planeOffset++)
        {
            if (planeOffset)
            {
                ...
                gTplGenericOpenRow.field.OPTION = (planeOffset == (pJob->plane - 1))? SEQ_OPTION_0280C8 : SEQ_OPTION_820080;
            }
            else
            {
                gTplGenericOpenRow.field.OPTION = (planeOffset == (pJob->plane - 1))? SEQ_OPTION_0280C8 : SEQ_OPTION_828080;
            }
            gTplGenericOpenRow.field.FD     = descOffset ? 0: 1;
...
static U16 DescGeneric_SNDK_MLC_PreOpen(tFIO_JOB* pJob)
{
    ...
    for (cnt = 0; cnt < pJob->paaCnt; cnt++)
    {
        ...
        die = GET_DIE_NUM_FROM_PAA(paa.b.plane, paa.b.lun, paa.b.ce, paa.b.ch);
        ...
        for (planeOffset = 0; planeOffset < pJob->plane; planeOffset++)
        {
            if (planeOffset)
            {
                ...
                actDie = GET_DIE_NUM_FROM_PAA(paa.b.plane, paa.b.lun, paa.b.ce, paa.b.ch);
                gTplGenericPreOpen.field.OPTION = (planeOffset == (pJob->plane - 1))? SEQ_OPTION_0280C8 : SEQ_OPTION_820080;
            }
            else
            {
                gTplGenericPreOpen.field.OPTION = SEQ_OPTION_828080;
            }
...
static U16 DescGeneric_SNDK_MLC_IdsRead(tFIO_JOB *pJob)
{
    ...
    for (cnt = 0; cnt < pJob->paaCnt; cnt++)
    {
        ...
        if (gOpenRow[actDie] != paa.rough.row)
        {
            gTplGenericIdsRead.field.OPTION = SEQ_OPTION_1480C8;
            gOpenRow[actDie] = paa.rough.row;
        }
        else
        {
...
static U16 DescGeneric_SNDK_MLC_IdsPageRead(tFIO_JOB *pJob)
{
    ...
    for (cnt = 0; cnt < totalCnt; cnt++)
    {
        ...
        if (gOpenRow[actDie] != paa.rough.row)
        {
            gTplGenericIdsPageRead.field.OPTION = SEQ_OPTION_1480C8;
            gOpenRow[actDie] = paa.rough.row;
        }
        else
        {
...
**/
//-----------------------------------------------------------------------------
static U16 DescGeneric_SNDK_MLC_NormalTwoBitDecode(tFIO_JOB *pJob)
{
    tPAA paa;
    U16  cnt;
    U8   fd, ju;
    ...
    for(cnt = 0; cnt < pJob->paaCnt; cnt++)
    {
        ...
        DescGeneric_SNDK_MLC_HD_Retry_CW(pJob, 0, fd, ju, paa);  // pjob, n-th CW, FD, JU

        /* CW0 SD 5C C5 + SOFT READ LEVEL SET */
        DescGeneric_SNDK_MLC_SD_Retry_CW(pJob, 0, paa);  //pjob, n-th CW

        /* CW 1 SD  5C C5 + SOFT READ LEVEL SET */
        DescGeneric_SNDK_MLC_HD_Retry_CW(pJob, 1, 0, 0, paa); // pjob, n-th CW, FD, JU

        DescGeneric_SNDK_MLC_SD_Retry_CW(pJob, 1, paa);   //pjob, n-th CW
    }
...
static __inline void DescGeneric_SNDK_MLC_InitTemplate(void)
{
   ...
   // OpenRow
    ...
    gTplGenericOpenRow.field.PREFIX_3B   = 0x5D;
    ...
    gTplGenericFastRead.field.PREFIX_3B   = 0x5D;
    gTplGenericFastRead.field.OPTION      = SEQ_OPTION_1480C8;
    ...
    gTplGenericReadRetry.field.PREFIX_3B   = 0x5D;
    ...
    gTplGenericPreOpen.field.PREFIX_3B   = 0x5D;
    ...
    gTplGenericIdsRead.field.PREFIX_3B   = 0x5D;
    gTplGenericIdsRead.field.OPTION      = SEQ_OPTION_1480C8;
    ...
    gTplGenericIdsPageRead.field.PREFIX_3B   = 0x5D;
