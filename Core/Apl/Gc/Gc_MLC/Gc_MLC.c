...
//-----------------------------------------------------------------------------
#if (_NAND_ == NAND_MLC && ENABLE_GC == TRUE)
...
void gc_setPaaLP(U8 p2lIdx, U8 lun, U8 ce)
{
    ...
    U8 fragOffset = 0;
    U8 paaCnt     = 0;
    U8 offSet     = GC_ECU_AU_PER_PAGE / GC_ECU_AU_PER_HALF_PAGE;

    #if (MAX_SUPPORTED_PLANES == 4)
    for (fragOffset = 0; fragOffset < offSet; fragOffset++)
    #endif
    {
        for (ch = 0; ch < GC_NF_NUM_CHANNELS; ch++)
        {
            for (frag = 0; frag < GC_ECU_AU_PER_HALF_PAGE; frag++)
            {
                gFastPaaBuf[paaCnt].b.ce    = ce;
                gFastPaaBuf[paaCnt].b.lun   = lun;
                gFastPaaBuf[paaCnt].b.ch    = ch;
                gFastPaaBuf[paaCnt].b.frag  = frag + (fragOffset * offSet);
                gFastPaaBuf[paaCnt].b.plane = 0;  // Storing all P2L info @ plane 0
...
void gc_issueJobLP(U8 curP2lIdx)
{
    ...
    U32       idx;
    U32       cacheIdx;
    U8        die, ch, lun, ce, planeH, planeL, pageIdx[8];
    ...
    #if (MAX_SUPPORTED_PLANES == 4)
    for (planeH = 0; planeH < GC_ECU_AU_PER_HALF_PAGE; planeH++)
    #endif
    {
        for (ch = 0; ch < GC_NF_NUM_CHANNELS; ch++)
        {
            for (planeL = 0; planeL < GC_ECU_AU_PER_HALF_PAGE; planeL++)
            {
                pTail = GET_SSD_PTR_FROM_INDEX(cacheIdx);

                // P2L Table
                die = FTL_GET_DIE_NUM(lun, ce, ch);
                idx = FTL_GET_LP_IDX(0, curP2lIdx, die, pageIdx[ch] * FTL_GET_LP_PAGE_PER_4K, 0, 0);

                pageIdx[ch]++;
                if (pageIdx[ch] * FTL_GET_LP_PAGE_PER_4K == GC_NF_PAGE_PER_BLK)
                    pageIdx[ch] = 0;

                pTail->startAddr = (U32)&gdwGCLastPageBuf[idx];       // P2L Table Start Address
                pTail->endAddr   = (pTail->startAddr + 0x1000 - 1);   // P2L Table End Address - 1;
    ...
    while ((gGCInfo.jobPaaCnt == 0) && (gcDone == FALSE))
    {
        ...
        if ((gGCInfo.jobPaaCnt == 0) && (scanPaaCnt >= GC_SCAN_PAA_CNT_TH))
        {
            if ((gGCppBuf[curP2lIdx].mode_Info.mode == GC_ST_FORCE) || (gGCppBuf[curP2lIdx].mode_Info.mode == GC_ST_STATIC_WL))
            {
                ...
                {
                    if (gGCppBuf[curP2lIdx].qos_Info.hostCacheTH > GC_SCAN_PAA_CNT_TH)
...
U8 GC_Get_NxtDie(U8 curP2lIdx)
{
    ...
    if (gGCppBuf[curP2lIdx].recyc_Info.curPaa.b.plane++ == (GC_NF_PLANE_PER_LUN - 1))
    {
        ...
        if (gGCppBuf[curP2lIdx].recyc_Info.curPaa.b.ce++ == (GC_NF_DEVICE_PER_CHANNEL - 1))
        {
            ...
            if (gGCppBuf[curP2lIdx].recyc_Info.curPaa.b.lun++ == (GC_NF_LUN_PER_DEVICE - 1))
            {
                ...
                if (gGCppBuf[curP2lIdx].recyc_Info.curPaa.b.page++ == (GC_NF_USER_PAGE_PER_BLK - 1))
                {
...
void GC_Test_ReadLP(U16 blk)
{
    ...
    offSet = GC_ECU_AU_PER_PAGE / GC_ECU_AU_PER_HALF_PAGE;
            ...
            #endif
            {
                for (ch = 0; ch < GC_NF_NUM_CHANNELS; ch++)
                {
                    for (planeL = 0; planeL < GC_ECU_AU_PER_HALF_PAGE; planeL++)
                    {
                        ...
                        // P2L Table
                        die = FTL_GET_DIE_NUM(lun, ce, ch);
                        idx = FTL_GET_LP_IDX(0, 0, die, pageIdx[ch]*FTL_GET_LP_PAGE_PER_4K, 0, 0);
        ...
        for (lun = 0; lun < GC_NF_LUN_PER_DEVICE; lun++)
        {
            for (ce = 0; ce < GC_NF_DEVICE_PER_CHANNEL; ce++)
            {
                for (ch = 0; ch < GC_NF_NUM_CHANNELS; ch++)
                {
                    ...
                    {
                        D_PRINTF("- LN[%d]CE[%d]CH[%d]PL[%2x]\n", lun, ce, ch, plane);
                        for (page = 0; page < GC_NF_USER_PAGE_PER_BLK; page++)
                        {
                            for (frag = 0; frag < GC_XFER_ORDER; frag++)
                            {
                                die = FTL_GET_DIE_NUM(lun, ce, ch);
                                idx = FTL_GET_LP_IDX(0, curP2lIdx, die, page, plane, frag);
                                phyPaa.b.ch    = ch;
                                phyPaa.b.ce    = ce;
                                phyPaa.b.lun   = lun;
