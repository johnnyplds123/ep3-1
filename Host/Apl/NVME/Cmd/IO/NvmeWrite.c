...
//-----------------------------------------------------------------------------
/// @brief NVMe I/O Command structure - Write
typedef struct
{
...
} NvmeIoWriteCommand_t;
COMPILE_ASSERT(sizeof(NvmeIoWriteCommand_t)==64, "NVMe write command size shall be 64");


//-----------------------------------------------------------------------------
//  Private function prototype definitions:
//-----------------------------------------------------------------------------
static void SetupWriteTransfer(HcmdQ_t* pHcmdQ);
static void WriteTransferDone(HcmdQ_t* pHcmdQ);
static void MediaWriteRequest(HcmdQ_t* pHcmdQ);
static void MediaWriteRequestDone(HcmdQ_t* pHcmdQ);
...
//-----------------------------------------------------------------------------
/**
    Seetup data transfer for write (host to device).
...
static void SetupWriteTransfer(HcmdQ_t* pHcmdQ)
{
    // set callback function for host data transfer done
    SetHcmdHandler(pHcmdQ, WriteTransferDone);
...
//-----------------------------------------------------------------------------
/**
    Write transfer done call back function.
      - called from data transfer done interrupt.
...
static void WriteTransferDone(HcmdQ_t* pHcmdQ)
{
    ...
    if (pHcmdQ->attr.b.autoEoC == FALSE)
    {
...
//-----------------------------------------------------------------------------
/**
    Write command - I/O 01h

    @param[in]  pHcmdQ   Host command queue pointer
**/
//-----------------------------------------------------------------------------
void NvmeCmd_Write(HcmdQ_t* pHcmdQ)
{
#if _TCG_!=TCG_NONE
    ...
    gNvmeIfMgr.hostWriteCommands++;
    gNvmeIfMgr.dataUnitsWritten += (pHcmdQ->addrInfo.nb << gNvmeIfMgr.ns[pHcmdQ->addrInfo.nsId0].hbu_shift);

    SetupWriteTransfer(pHcmdQ);

    AplCmdSensor_WriteCmdDetect(pHcmdQ);
}
