...
Error_t HalIrq_OneTimeInit(InitBootMode_t initMode)
{
    ...
    VIC_CLEAR_STATUS(1, U32_ALL_BIT_SET); // clear interrupts status
    VIC_DISABLE(1, U32_ALL_BIT_SET);      // Disable all interrupts
    ...
    {
        VIC_SET_HANDLER(1, irqBitNumber, DefaultHwIrqHandler);
    }

    // register IRQ for internal shared module
    VIC_SET_HANDLER(1, cIrqSourceTimer, HalTimer_IrqHandle);    // Register timer irq handler
    VIC_SET_HANDLER(1, cIrqSourceIpc1, HalIpc_IrqHandle);       // Register IPC irq handler

    // enable IRQ for internal shared module
    VIC_ENABLE(1, BIT(cIrqSourceTimer) | BIT(cIrqSourceIpc1));
...
Error_t HalIrq_SetPowerMode(IrqPowerMode_t powerMode)
{
    ...
    case cIrqPmDeepSleep:
        ///< Save VIC Irq status
        saveIrqStatus.vicIrq.enableStatus = VIC_GET_ENABLE_STATUS(1);
        for(index=0; index<32; index++)
        {
            saveIrqStatus.vicIrq.vectorAddr[index] = VIC_GET_HANDLER(1, index);
        }
        ///< Save HW Irq status
        for(irqId=cIrqBegin; irqId<cIrqNumOfId; irqId++)
        {
    ...
    case cIrqPmActive:
        if ((currentIrqPowerMode == cIrqPmSleep) || (currentIrqPowerMode == cIrqPmDeepSleep))
        {
            ///< Restore VIC Irq status
            for(index=0; index<32; index++)
            {
                VIC_SET_HANDLER(1, index, saveIrqStatus.vicIrq.vectorAddr[index]);
            }
            VIC_ENABLE(1, saveIrqStatus.vicIrq.enableStatus);
...
IrqHandler_t HalIrq_RegisterHandler(IrqId_t irqId, IrqHandler_t irqHandler, uint32_t state)
{
    ...
    {
    case cIrqMediaDone:
        VIC_SET_HANDLER(1,  cIrqSourceFlash, irqHandler);
        break;
    }
...
uint32_t HalIrq_EnDisable(IrqId_t irqId, uint32_t state)
{
    ...
    orgState = VIC_IS_ENABLED(1, BIT(irqBitNumber));

    if(state == ENABLE)
    {
        VIC_ENABLE(1, BIT(irqBitNumber));
    }
    else
    {
        VIC_DISABLE(1, BIT(irqBitNumber));
    }
...
uint32_t HalIrq_Handler(void)
{
    ...
    while (VIC_READ_STATUS(1))
    {
        irqHandler = (IrqHandler_t)(VIC_GET_ACTIVE_HANDLER(1));
        irqHandler((uint32_t)sIrqHandler);
        VIC_SERVICED(1); // clear the respective interrupt
